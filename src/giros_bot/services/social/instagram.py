...
class InstagramPublisher(ISocialPublisher):
    ...
    Requiere un flujo de 2 pasos:
      1. Crear contenedor de media (Meta descarga la imagen desde payload.image_url)
      2. Publicar el contenedor

    La imagen se provee vía presigned URL de Cloudflare R2 (con watermark),
    igual que Facebook, garantizando consistencia visual entre ambas plataformas.
    Meta descarga la imagen desde esa URL al procesar el contenedor.
    """
    ...
    async def publish(self, payload: SocialPayload) -> PublishResult:
        ...
        if not payload.image_url:
            return PublishResult(
                platform=self.platform_name,
                success=False,
                error_message="No hay URL de imagen disponible para publicar en Instagram.",
            )

        base_url = f"https://graph.facebook.com/v21.0/{settings.instagram_account_id}"
        caption = (
            f"{payload.social_assets.instagram_copy}\n\nLink en la bio o visita: {payload.post_url}"
        )

        try:
            async with httpx.AsyncClient(timeout=30) as client:
                # PASO 1: Crear el contenedor de Media
                # Meta descarga la imagen desde payload.image_url (presigned URL de R2)
                ...
                logger.info(
                    "InstagramPublisher: Contenedor creado (ID: %s). Esperando procesamiento...",
                    creation_id,
                )

                # Pausa para que Meta procese la imagen antes de publicar
